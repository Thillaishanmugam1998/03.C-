...
public class OrderService : IOrderService
{
    ...
    public IReadOnlyList<Order> GetByFilters(GetOrdersQueryRequest request)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.Status))
        {
            query = query.Where(order =>
                order.Status.Equals(request.Status.Trim(), StringComparison.OrdinalIgnoreCase));
        }

        if (!string.IsNullOrWhiteSpace(request.CustomerName))
        {
            query = query.Where(order =>
                order.CustomerName.Contains(request.CustomerName.Trim(), StringComparison.OrdinalIgnoreCase));
        }
